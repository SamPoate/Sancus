...
import { IUser, IMember, IPartner } from '../types';
...
const AdminUserList: React.FC<UserListProps> = () => {
    ...
    const { admin, members, partners } = useSelector(
    ...
    return (
        <Table inverted selectable>
            ...
            <Table.Body>
                {admin.users.map((listedUser: IUser) => {
                            ...
                            <Table.Row key={listedUser.id}>
                                ...
                                <Table.Cell>
                                    <Dropdown
                                        ...
                                        onChange={(_, { value }) =>
                                            value &&
                                            setAssignedMember(value.toString())
                                        }
                                        options={members.map(
                                            (member: IMember) => ({
                                                value: member.id,
                                ...
                                <Table.Cell>
                                    <Dropdown
                                        ...
                                        onChange={(_, { value }) =>
                                            value &&
                                            setAssignedPartner(value.toString())
                                        }
                                ...
                                <Table.Cell>
                                    <Dropdown
                                        value={assignedRole || userEditor.role}
                                        onChange={(_, { value }) =>
                                            value &&
                                            setAssignedRole(value.toString())
                                        }
                                        options={[
                                            {
                                                value: 'admin',
                    ...
                    return (
                        <Table.Row key={listedUser.id}>
                            ...
                            <Table.Cell>
                                {listedUser.memberId
                                    ? memberDictionary[listedUser.memberId].name
                                    : 'Unassigned'}
                            </Table.Cell>
                            <Table.Cell>
                                {listedUser.partnerId
                                    ? partnerDictionary[listedUser.partnerId]
                                          .name
                                    : 'Unassigned'}
                            </Table.Cell>
                            ...
                            <Table.Cell textAlign='center'>
                                <Icon
                                    ...
                                    color='blue'
                                    size='large'
                                    onClick={() => setUserEditor(listedUser)}
                                />
                            </Table.Cell>
                        </Table.Row>
                    );
